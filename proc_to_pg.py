...
def migrate_code(code: str) -> str:
    ...
    # -------------------------------------------------
    # 4. Remove m_RunTimeCntxt->sqlErrorCode  check block
    # -------------------------------------------------
    code = re.sub(
        r'\s*m_RunTimeCntxt->sqlErrorCode\s*',
        '',
        code,
        flags=re.DOTALL
    )
     # -------------------------------------------------
    # 4.6 Remove specific logger line
    # -------------------------------------------------
    code = re.sub(
        r'^.*loggerObj\.log\(LM_DEBUG,_FILE,LINE,0,0,".*$\n?',
        '',
        code,
        flags=re.MULTILINE
    )
    # -------------------------------------------------
    # 5 Remove lines containing ProcessInitializer::g_ConnectionPool
    # -------------------------------------------------
    code = re.sub(
        r'^.*ProcessInitializer::g_ConnectionPool.*$\n?',
        '',
        code,
        flags=re.MULTILINE
    )
    ...
    def replace_sql(match):
        ...
        return (
            f'string query = " {sql} ;";\n'
            f'int recordCount = DbUtil::executeNonQuery(query);'
        )
    ...
    # -------------------------------------------------
    # 8. Remove remaining EXEC SQL statements
    # -------------------------------------------------
    code = re.sub(
        r'^\s*EXEC\s+SQL.*?;\s*$',
        '',
